...
class order_partnum_etc extends StatefulWidget {
  const order_partnum_etc({Key? key}) : super(key: key);

  @override
  State<order_partnum_etc> createState() => _order_partnum_etcState();
}

class _order_partnum_etcState extends State<order_partnum_etc> {
  Future<List<GetallpartnumETC>>? getAllpartnumETCData;
  final date = new DateFormat('dd/MM/yyyy');
  List listPartnumETCData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllpartnumETCData = GetpartnumAPI.getviewAllpartnumETC();
    });
  }

  deleteItem() {
    if (listPartnumETCData.length > 10) {
      setState(() {
        getAllpartnumETCData?.then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllpartnumETCData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                print(snapshot.data.length);
                listPartnumETCData = snapshot.data;
                print('PartnumETC');
