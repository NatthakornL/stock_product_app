...
class order_partnum_sam extends StatefulWidget {
  const order_partnum_sam({Key? key}) : super(key: key);

  @override
  State<order_partnum_sam> createState() => _order_partnum_samState();
}

class _order_partnum_samState extends State<order_partnum_sam> {
  Future<List<GetallpartnumSAM>>? getAllpartnumSAMData;
  final date = new DateFormat('dd/MM/yyyy');
  List listPartnumSAMData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllpartnumSAMData = GetpartnumAPI.getviewAllpartnumSAM();
    });
  }

  deleteItem() {
    if (listPartnumSAMData.length > 10) {
      setState(() {
        getAllpartnumSAMData?.then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllpartnumSAMData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                print(snapshot.data.length);
                listPartnumSAMData = snapshot.data;
                print('PartnumSAM');
