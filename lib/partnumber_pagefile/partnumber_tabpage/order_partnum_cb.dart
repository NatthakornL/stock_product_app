...
class order_partnum_cb extends StatefulWidget {
  const order_partnum_cb({Key? key}) : super(key: key);

  @override
  State<order_partnum_cb> createState() => _order_partnum_cbState();
}

class _order_partnum_cbState extends State<order_partnum_cb> {
  Future<List<GetallpartnumCB>>? getAllpartnumCBData;
  final date = new DateFormat('dd/MM/yyyy');
  List listPartnumCBData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllpartnumCBData = GetpartnumAPI.getviewAllpartnumCB();
    });
  }

  deleteItem() {
    if (listPartnumCBData.length > 10) {
      setState(() {
        getAllpartnumCBData?.then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllpartnumCBData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                print(snapshot.data.length);
                listPartnumCBData = snapshot.data;
                print('PartnumCB');
