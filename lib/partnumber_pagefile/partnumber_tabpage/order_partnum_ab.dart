...
class order_partnum_ab extends StatefulWidget {
  const order_partnum_ab({Key? key}) : super(key: key);

  @override
  State<order_partnum_ab> createState() => _order_partnum_abState();
}

class _order_partnum_abState extends State<order_partnum_ab> {
  Future<List<GetallpartnumAB>>? getAllpartnumABData;
  final date = new DateFormat('dd/MM/yyyy');
  List listPartnumABData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllpartnumABData = GetpartnumAPI.getviewAllpartnumAB();
    });
  }

  deleteItem() {
    if (listPartnumABData.length > 10) {
      setState(() {
        getAllpartnumABData?.then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllpartnumABData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                print(snapshot.data.length);
                listPartnumABData = snapshot.data;
                print('PartnumAB');
