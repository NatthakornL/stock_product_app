...
class order_partnum_enm extends StatefulWidget {
  const order_partnum_enm({Key? key}) : super(key: key);

  @override
  State<order_partnum_enm> createState() => _order_partnum_enmState();
}

class _order_partnum_enmState extends State<order_partnum_enm> {
  Future<List<GetallpartnumENM>>? getAllpartnumENMData;
  final date = new DateFormat('dd/MM/yyyy');
  List listPartnumENMData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllpartnumENMData = GetpartnumAPI.getviewAllpartnumENM();
    });
  }

  deleteItem() {
    if (listPartnumENMData.length > 10) {
      setState(() {
        getAllpartnumENMData?.then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllpartnumENMData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                print(snapshot.data.length);
                listPartnumENMData = snapshot.data;
                print('PartnumENM');
