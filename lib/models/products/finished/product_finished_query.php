...
class ProductFinishedQuery
{

    public static function getRawDoneQuery()
    {
        return "SELECT id, so_id, detail_ref, partnumber, ts_name, qty, doc_no, qty_receive, issueing_ref,
        (
            SELECT DISTINCT a1.issueing_no
            FROM Sys_WarehouseManagement..sys_warehouse_issueing AS a1
            LEFT JOIN Sys_WarehouseManagement..sys_warehouse_issueing_detail AS a2 ON a1.issueing_id = a2.issueing_id
            WHERE a1.IsDelete = 0
            AND a2.IsDelete = 0
            AND a2.tran_ref = innerTable.tran_ref
        ) AS issueing_no
        FROM (
            SELECT *,
            (
        ...
        WHERE (
            YEAR(ts_create) = YEAR(GETDATE())
            AND MONTH(ts_create) in (
            MONTH(GETDATE()),
            MONTH(GETDATE()) -1
            )
        )
        ORDER BY ts_receive DESC
        ";
    }
    ...
    {
        return "ORDER BY ts_receive DESC";
    }

    public static function getRawNotDoneQuery()
    {
        return "SELECT DATEDIFF(DAY, GETDATE(), ts_create) AS getcre_diff,
        id, part, type_id, so_id, detail_ref, tran_ref, partnumber, ts_name, qty, doc_no, issueing_ref, qty_receive,
        (
            SELECT DISTINCT a1.issueing_no
            FROM Sys_WarehouseManagement..sys_warehouse_issueing AS a1
            LEFT JOIN Sys_WarehouseManagement..sys_warehouse_issueing_detail AS a2 ON a1.issueing_id = a2.issueing_id
    ...
    {
        return "SELECT DATEDIFF(DAY, GETDATE(), ts_create) AS getcre_diff,
        id, part, type_id, so_id, detail_ref, tran_ref, partnumber, ts_name, qty, doc_no, issueing_ref,
        (
            SELECT DISTINCT a1.issueing_no
            FROM Sys_WarehouseManagement..sys_warehouse_issueing AS a1
            LEFT JOIN Sys_WarehouseManagement..sys_warehouse_issueing_detail AS a2 ON a1.issueing_id = a2.issueing_id
