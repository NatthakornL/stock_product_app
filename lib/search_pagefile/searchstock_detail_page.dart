// ignore_for_file: camel_case_types, prefer_const_constructors, unused_import, unused_element, prefer_typing_uninitialized_variables, sized_box_for_whitespace, avoid_unnecessary_containers, avoid_print
...
class searchstock_detail_page extends StatefulWidget {
  const searchstock_detail_page({Key? key}) : super(key: key);

  @override
  State<searchstock_detail_page> createState() =>
      _searchstock_detail_pageState();
}

class _searchstock_detail_pageState extends State<searchstock_detail_page> {
  Future<List<GetallsearchstockDetail>>? getAllSearchStockDetailData;
  List listSearchStockDetailData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllSearchStockDetailData =
          GetsearchstockAPI.getviewAllsearchstockDeatil();
    });
  }

  deleteItem() {
    if (listSearchStockDetailData.length > 10) {
      setState(() {
        getAllSearchStockDetailData?.then((value) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllSearchStockDetailData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done &&
                  snapshot.hasData) {
                print(snapshot.data.length);
                listSearchStockDetailData = snapshot.data;
                print('SearchStockDetail');
