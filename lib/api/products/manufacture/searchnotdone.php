...
include_once '../../../models/products/manufacture/product_manufacture_model.php';
...
try {
    if (isset($_GET['searchText']) && isset($_GET['page']) && isset($_GET['pageSize'])) {
        $result['page'] = $_GET['page'];
        $result['pageSize'] = $_GET['pageSize'];

        $OFFSET = $result['page'] * $result['pageSize'];

        Database::connectToSysWorkingTag();
        $result['dataCount'] = ProductManufactureModel::getManufactureNotDoneSearchCount($_GET['searchText']);
        $result['pagination'] = round($result['dataCount'] / $result['pageSize']);
        $result['data'] = ProductManufactureModel::getManufactureNotDoneSearch($_GET['searchText'], $OFFSET, $result['pageSize']);
