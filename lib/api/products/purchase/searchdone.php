...
try {
    if (isset($_GET['searchText']) && isset($_GET['page']) && isset($_GET['pageSize'])) {
        ...
        $result['dataCount'] = ProductPurchaseModel::getPurchaseDoneSearchCount($_GET['searchText']);
        $result['pagination'] = round($result['dataCount'] / $result['pageSize']);
        $result['data'] = ProductPurchaseModel::getPurchaseDoneSearch($_GET['searchText'], $OFFSET, $result['pageSize']);
