...
try {
    if (isset($_GET['searchText']) && isset($_GET['page']) && isset($_GET['pageSize'])) {
        ...
        $result['dataCount'] = ProductPurchaseModel::getPurchaseNotDoneSearchCount($_GET['searchText']);
        $result['pagination'] = round($result['dataCount'] / $result['pageSize']);
        $result['data'] = ProductPurchaseModel::getPurchaseNotDoneSearch($_GET['searchText'], $OFFSET, $result['pageSize']);
