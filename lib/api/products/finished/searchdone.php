...
try {
    if (isset($_GET['searchText']) && isset($_GET['page']) && isset($_GET['pageSize'])) {
        ...
        $result['dataCount'] = ProductFinishedModel::getFinishedDoneSearchCount($_GET['searchText']);
        $result['pagination'] = round($result['dataCount'] / $result['pageSize']);
        $result['data'] = ProductFinishedModel::getFinishedDoneSearch($_GET['searchText'], $OFFSET, $result['pageSize']);
