...
class buystock_page_etc extends StatefulWidget {
  const buystock_page_etc({Key? key}) : super(key: key);

  @override
  State<buystock_page_etc> createState() => _buystock_page_etcState();
}

class _buystock_page_etcState extends State<buystock_page_etc> {

  Future<List<GetallbuystockETC>>? getAllbuystockETCData;
  final date = new DateFormat('dd/MM/yyyy');
  List listBuystockETCData = [];

  get index => context;
  getpsd() {
    setState(() {
      getAllbuystockETCData = GetbuystockAPI.getviewAllbuystockETC();
    });
  }

  // deleteItem() {
  //   if (listBuystockData.length > 10) {
  //     setState(() {
  //       getAllbuystockData?.then((value) {
  ...
  //         context,
  //         MaterialPageRoute(
  //             builder: (context) => buystock_detail_page()));
  //   }
  // }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        child: Center(
          child: FutureBuilder(
            future: getAllbuystockETCData,
            builder: (BuildContext context, AsyncSnapshot snapshot) {
              if (snapshot.connectionState == ConnectionState.done && snapshot.hasData) {
                print(snapshot.data.length);
                listBuystockETCData = snapshot.data;
                print('BuystockETC');
